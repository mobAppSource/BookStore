...
RENAME TABLE `apps_countries` TO 'countries';

ALTER TABLE Book_Author DROP FOREIGN KEY Book_Author_Authors;
ALTER TABLE `bookstore`.`Authors` 
CHANGE COLUMN `author_id` `author_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`Book_Author` 
CHANGE COLUMN `Book_Author_ID` `Book_Author_ID` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`Books` 
DROP FOREIGN KEY `Books_Publishers`,
DROP FOREIGN KEY `Books_Book_Genre`;

ALTER TABLE `bookstore`.`Book_Genre` 
CHANGE COLUMN `book_genreid` `book_genreid` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`Book_Images` 
CHANGE COLUMN `imageID` `imageID` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`Order` 
DROP FOREIGN KEY `Order_User`;
ALTER TABLE `bookstore`.`Order` 
CHANGE COLUMN `order_id` `order_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`Order_Details` 
DROP FOREIGN KEY `Order_Details_Order`,
DROP FOREIGN KEY `Order_Details_Books`;

ALTER TABLE `bookstore`.`Publishers` 
CHANGE COLUMN `publisher_id` `publisher_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`User_Billing` 
DROP FOREIGN KEY `User_Billing_User`;

ALTER TABLE `bookstore`.`Order` 
CHANGE COLUMN `order_id` `order_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`Order_Details` 
CHANGE COLUMN `order_detail_id` `order_detail_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`User` 
CHANGE COLUMN `user_id` `user_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`User_Billing` 
CHANGE COLUMN `billing_id` `billing_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

ALTER TABLE `bookstore`.`Book_Author` 
DROP FOREIGN KEY `Book_Author_Books`;

ALTER TABLE `bookstore`.`Book_Images` 
DROP FOREIGN KEY `Book_Images_Books`;

ALTER TABLE `bookstore`.`Books` 
CHANGE COLUMN `book_id` `book_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '' ;

INSERT INTO `bookstore`.`Authors` (`author_id`, `author_firstname`, `author_lastname`, `author_othernames`, `author_birthdate`, `author_history`, `author_image`, `author_isactive`) VALUES ('', 'John', 'Otaalo', 'Chrispine', '1990-03-24', 'Something about the author that does not have to do a lot there with', 'https://yt3.ggpht.com/-iLEaGGRXaY8/AAAAAAAAAAI/AAAAAAAAAAA/Ycv1cGgDaGk/s900-c-k-no/photo.jpg', '1'), ('', 'Alexis', 'Sanchez', 'Gulliermo', '1989-02-05', 'He is a very prolific writer and footballer', 'http://i1.mirror.co.uk/incoming/article3841911.ece/ALTERNATES/s615/PAY-Alexis-Sanchez.jpg', '1');

INSERT INTO `bookstore`.`Publishers` (`publisher_name`, `publisher_country`, `publisher_city`, `publisher_active`) VALUES ('LongHorn Publishers', '1', 'Test', '1');
INSERT INTO `bookstore`.`Publishers` (`publisher_name`, `publisher_country`, `publisher_city`, `publisher_active`) VALUES ('Kilimanjaro Publishers', '244', 'Another City', '1');

INSERT INTO `bookstore`.`Book_Genre` (`book_genre`) VALUES ('Fiction');
INSERT INTO `bookstore`.`Book_Genre` (`book_genre`) VALUES ('Horror');
INSERT INTO `bookstore`.`Book_Genre` (`book_genre`) VALUES ('Fairy Tales');